...
enum SOLVE_SQUARE_RESULT solve_square (FILE* const stream_err, const double a, const double b, const double c, \
                                      double * const x1, double * const x2)
{
    ...
    if (is_null (a))
    {
        Log ({stream_err, __FILE__, __FUNCTION__, __LINE__}, INFO,
              "The program started solving lineal equation\n");
    ...
    if (is_null (d))
    {
        Log ({stream_err, __FILE__, __FUNCTION__, __LINE__}, INFO, "The program got zero discriminant\n");
        *x1 = -b / (2 * a);
    ...
    if (d < 0)
    {
        Log ({stream_err, __FILE__, __FUNCTION__, __LINE__}, WARNING,
              "The program got negative discriminant\n");
        return NO_ROOTS;
    }
