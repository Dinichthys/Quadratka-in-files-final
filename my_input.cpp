...
bool input_coefficients (FILE * stream_in, FILE * stream_out, FILE * stream_err,  \
                        double * const a, double * const b, double * const c)
{
    ...
    while (tries < MAX_TRIES)
    {
        ...
        {
            Log ({stream_err, __FILE__, __FUNCTION__, __LINE__}, INFO, "The program got coefficients\n");
            return true;
        }
        if (d == EOF)
        {
    ...
    Log ({stream_err, __FILE__, __FUNCTION__, __LINE__}, INFO, "The program didn't get coefficients\n");
